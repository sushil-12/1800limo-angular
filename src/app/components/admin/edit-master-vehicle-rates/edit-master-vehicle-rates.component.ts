...
@Component({
	selector: 'app-edit-master-vehicle-rates',
	templateUrl: './edit-master-vehicle-rates.component.html',
	styleUrls: ['./edit-master-vehicle-rates.component.scss']
})
export class EditMasterVehicleRatesComponent implements OnInit
{
	public tree: any;
	...
	ngOnInit(): void
	{
		...
		this.addVehicleRatesForm = this.formBuilder.group({
			...
			minimum_cruise_port_departure_rate: ['', [Validators.pattern("^[0-9]*(\.[0-9]+)?$")]],
		...
		this.adminService.getMasterVehicleInfo(this.vehicleId)
			.pipe(
				catchError(err => 
				{
			...
			).subscribe(({ data }: any) =>
			{
				...
				//get vehicle rates
				this.adminService.getMasterVehicleRates(this.vehicleId)
					.pipe(
						catchError(err =>
						{
							this.stateManagementService.setprogressBar(false);
					...
					{
						console.log(result, "?????????")
						if (result.data.amenities_rates)
						{
							Object.entries(result.data.amenities_rates).forEach(
								([key, value]) =>
								{
	...
	submitForm()
	{
		...
		this.adminService.editMasterVehicleRates(this.addVehicleRatesForm.value)
			.pipe(
				catchError(err =>
				{
	...
	setFormValue(form_control: string, value: any)
	{
		...
		if (typeof value == 'number' && (Math.round(value * 100) < 0 || Math.round(value * 100) > 100000))
		{
			return
		}
