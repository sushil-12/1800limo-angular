...
export class AffiliateStep1Component implements OnInit {
	...
	ngOnInit(): void {
		...
		this.addAffiliateAccountForm = this.formBuilder.group({
			...
			Email: ['', [Validators.required,Validators.pattern(/^[a-zA-Z0-9.]+@[a-zA-Z0-9.]+\.[a-zA-Z]{2,}$/i)]],
			latitude: [''],
			longitude: [''],
			...
			DBA: [''],
			dispatchEmail: ['', [Validators.email,Validators.pattern(/^[a-zA-Z0-9.]+@[a-zA-Z0-9.]+\.[a-zA-Z]{2,}$/i)]],
			Dispatch: [''],
			DispatchIsd: ['+1'],
			DispatchCountry: ['us'],
			BusinessFrontPhoto: [''],
