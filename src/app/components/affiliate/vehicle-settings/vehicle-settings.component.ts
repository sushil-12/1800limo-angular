...
export class VehicleSettingsComponent implements OnInit
{
	...
	public charter_options: Array<Object>;
	...
	ngOnInit(): void
	{
		...
		this.$api.charterOptions.subscribe((data: any) =>
		{
			this.charter_options = data;
		});
		...
		this.addVehicleForm = this.$form.group({
			...
			typeOfService: this.$form.array([], [Validators.required]),
			amenities: this.$form.array([], [Validators.required]),
			specialAmenitiesGet: this.$form.array([]),
			specialAmenities: this.$form.array([]),
			vehicleInteriorGet: this.$form.array([], [Validators.required]),
			vehicleInterior: this.$form.array([], [Validators.required]),
		...
		this.$api.getFieldsData()
			...
			).subscribe(result =>
			{
				...
				this.$api.getVehicleData(this.vehicleId)
					.pipe(
						catchError(err =>
						{
							this.stateManagementService.setprogressBar(false);
	...
	onFileChange(event, imageId, imageNumber)
	{
		...
		if (event.target.files && event.target.files.length)
		{
			...
			reader.onload = () =>
			{
				...
				this.$api.uploadVehicleImage(this.imageSrc)
					.pipe(
						catchError(err =>
						{
							this.stateManagementService.setprogressBar(false);
	...
	submitForm()
	{
		...
		this.$api.editVehicle(this.addVehicleForm.value)
			.pipe(
				catchError(err =>
				{
	...
	pushValuesTypeOfService(value: Array<any>)
	{
		...
		value.forEach((item: string) =>
		{
			...
			this.typeOfService.push(this.$form.control(item))
		})
	}
	...
	// The Radio Button Functionality
	service: Array<any> = []
