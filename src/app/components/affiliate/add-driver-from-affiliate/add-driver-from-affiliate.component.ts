...
export class AddDriverFromAffiliateComponent implements OnInit, AfterViewInit, AfterViewChecked
{
	...
	ngOnInit(): void
	{
		...
		this.affiliateService.driverDressLanguage()
			.pipe(
				catchError(err =>
				{
			...
			).subscribe(({ data }: any) =>
			{
				...
				if (this.affiliateId)
				{
					if (this.driverId)
					{
						...
						this.affiliateService.getDriver(this.driverId)
							.pipe(
								catchError(err =>
								{
									this.spinner.hide() //hide spinner
	...
	submitForm()
	{
		...
		this.addDriverForm.value.stepCompleted = this.affiliateService.getUpdatedStepsLocal('4');
		this.spinner.show();// show spinner
		this.disableSubmitButton = true; //disable submit button

		this.affiliateService.addDriver(this.addDriverForm.value)
			.pipe(
				catchError(err =>
				{
					this.spinner.hide();// hide spinner
