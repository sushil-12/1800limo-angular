...
@Component({
	selector: 'app-duplicate-vehicle',
	templateUrl: './duplicate-vehicle.component.html',
	styleUrls: ['./duplicate-vehicle.component.scss']
})
export class DuplicateVehicleComponent implements OnInit, AfterViewChecked {
			...
			.pipe(
				...
				this.affiliateService.getVehicleData(this.duplicateVehicleId)
					.pipe(
						...
						this.pushValuesTypeOfService(this.response2.data.typeOfService) // push values into form
	...
	submitForm() {
		...
		this.affiliateService.duplicateVehicle(this.addVehicleForm.value)
			.pipe(
				catchError(err => {
			...
			.subscribe(result => {
				...
				this.router.navigate(['/affiliate/step5/duplicate-vehicle-rates'], { queryParams: { vehicleId: this.response.data.id, duplicateVehicleId: this.duplicateVehicleId } });
			});
	}
