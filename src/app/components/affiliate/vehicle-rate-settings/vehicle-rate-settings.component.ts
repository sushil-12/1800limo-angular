	...
	changeDetection = {
		radioButton: (form_control: string, value: any) => {
			...
			if (value == "kilometer") {
				...
				this.VehicleRateSettingsForm.get('kilometer_rate')?.setValidators([Validators.required, Validators.pattern("^[0-9]*(\.[0-9]+)?$"), Validators.min(1.72)]); // Set back the validator
