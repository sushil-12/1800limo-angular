...
export class CreateNewBookingComponent implements OnInit
{
	...
	vehicle_details: Array<Array<string>> = []
	modal_data: any
	...
	ngOnInit()
	{
		...
		this.vehicle_selected = sessionStorage.getItem('selected_vehicle') ? JSON.parse(sessionStorage.getItem('selected_vehicle')) : null
		this.vehicle_details = [['vehicle type', 'vehice_type_id'], ['name', 'name'], ['amenities', 'amenities'], ['luggage', 'luggage'], ['passenger', 'passenger']]
	...
	openModal(data: any, charge: boolean)
	{
		if (charge)
		{
			this.modal_data = data.filter((item: any) => item.chargeable == 'yes')
		} else
		{
			this.modal_data = data.filter((item: any) => item.chargeable == 'no')
		}
	}
	...
	prefillBookingFormGroup()
	{
		...
		{
			this.affiliateService.getBookingDataForEdit(this.booking_id).subscribe((response: any) =>
			{
				...
				this.bookingForm.patchValue({
					...
					passenger_cell_isd: response.data.passenger_cell_isd != null ? response.data.passenger_cell_isd : '',
	...
	getBigData()
	{
		...
		{
			if (this.affiliateService.getBigData() != undefined)
			{
				this.initMap()
				setTimeout(() =>
				{
				...
				resolve(this.affiliateService.getBigData())
			}
			else
			{
	...
	fetchUserList(account_type: string)
	{
		...
		this.affiliateService.getAccountBytype(account_type).subscribe((response: any) =>
		{
	...
	{
		this.fetched_user = {}
		this.affiliateService.chooseUser(user_id, this.Form.account_type.value).subscribe((response: any) =>
		{
	...
	getVehiclesandDriversFromAffiliate(affiliate_id: number)
	{
		...
		this.affiliateService.affiliateVehicleList().then((response: any) =>
		{
	...
	submitForm(preview: boolean = false)
	{
		...
		if (preview)
		{
			...
			this.affiliateService.createBooking(this.bookingForm.value).subscribe((response: any) =>
			{
				if (response)
				{
					this._spinner.hide('savespinner')
					this._router.navigate(['/affiliate/my-bookings'])
					return
					// 	, {
					// 	queryParams: {
