...
@Component({
  selector: 'app-sub-affiliate-accounts',
  templateUrl: './sub-affiliate-accounts.component.html',
  styleUrls: ['./sub-affiliate-accounts.component.scss']
})
export class SubAffiliateAccountsComponent implements OnInit {
  ...
  ngOnInit(): void {
		this.searchText = localStorage.getItem('subAffiliateAccountSearch') ? localStorage.getItem('subAffiliateAccountSearch') : '' 
	...
	handleSearchKeyword(text:any){
		...
		this.timer = setTimeout(() => {
			localStorage.setItem('subAffiliateAccountSearch' , text)
  ...
  loadClientAccounts(pageUrl=null)
  {
      ...
      if(pageUrl){
        ...
        this.scroll('sub_affiliate_accounts')
      }
      this.spinner.show();
      ...
      this.affiliateService.getsubAffAccounts(pageUrl,keyword).then(result=>{
  ...
  enableDisableClicked(event,id)
  {
    ...
    this.affiliateService.subAffAccountStatus(id,status)
    .pipe(
        catchError(err => {
          this.spinner.hide();//hide spinner
