...
export class Step3Component implements OnInit, AfterViewInit
{
	...
	ngOnInit(): void
	{
		...
		if (this.affiliateId)
		{
			if (stepCompleted.includes('3'))
			{
				...
				this.affiliateService.getInsuranceDetail(this.affiliateId)
					.pipe(
						catchError(err =>
						{
	...
	submitForm()
	{
		...
		this.addInsuranceForm.value.stepCompleted = this.affiliateService.getUpdatedStepsLocal('3');

		this.spinner.show();
		this.disableSubmitButton = true; //disable submit button

		this.affiliateService.addInsuranceDetail(this.addInsuranceForm.value)
			.pipe(
				catchError(err =>
				{
					this.spinner.hide();//hide spinner
