import { Component, ElementRef, OnInit, ViewChild, isDevMode, AfterViewInit, ViewChildren, QueryList } from '@angular/core';
...
export class CreateNewBookingComponent implements OnInit, AfterViewInit {
	...
	@ViewChild('pickupAirportInput') pickupAirportInput!: ElementRef<HTMLInputElement>
	@ViewChild('dropoffAirportInput') dropoffAirportInput!: ElementRef<HTMLInputElement>
	@ViewChild('returnPickupAirportInput') returnPickupAirportInput!: ElementRef<HTMLInputElement>
	@ViewChild('returnDropoffAirportInput') returnDropoffAirportInput!: ElementRef<HTMLInputElement>
	...
	calculateBookingTotal(rateArray: any, serviceType: string, numberOfVehicles: number = 1) {
		...
		if (!rateArray) return { subtotal: 0, agentShare: 0 };

		/* -------- ALL INCLUSIVE TOTAL (Base for Taxes) -------- */
		let all_inclusive_total = 0
		for (let key in rateArray?.all_inclusive_rates || {}) {
			if (key == 'Base_Rate' && serviceType == 'charter_tour' && !this.min_rate_involved) continue
		...
		let base_rate_for_share = all_inclusive_total
		for (let key in rateArray?.amenities || {}) {
			base_rate_for_share += rateArray.amenities[key]?.baserate || 0
		}

		/* -------- ADMIN / AGENT SHARE PERCENT -------- */
		let admin_share_percent = 25
		if (this.BookingForm.value?.account_type == 'travel_planner' && !this.isCreatedByAdmin) {
			admin_share_percent = 15
			agentShare = base_rate_for_share * 0.10
		} else if (
			(this.updateType == 'repeat' || this.updateType == 'return' || this.updateType == 'round') &&
			this.currentUser?.created_by_role != 'subscriber'
		) {
			admin_share_percent = 15
			agentShare = base_rate_for_share * 0.10
		...
		let extra_gratuity = rateArray?.misc?.Extra_Gratuity?.amount || 0
		adminShare = (base_rate_for_share * admin_share_percent / 100) + (extra_gratuity * 0.25)

		/* -------- FINAL SUB-TOTAL CALCULATION -------- */
		subtotal = all_inclusive_total

		// Add Amenities
		for (let key in rateArray?.amenities || {}) {
			subtotal += rateArray.amenities[key]?.amount || 0
		}

		// Add Taxes (with Percentage support)
		const taxKeys = ['taxes', 'direct_taxes'];
		taxKeys.forEach(taxKey => {
			for (let key in rateArray?.[taxKey] || {}) {
				const tax = rateArray[taxKey][key]
				if (tax?.type === 'percent') {
					subtotal += (tax.baserate / 100) * all_inclusive_total
				} else {
					subtotal += tax?.amount || 0
				}
			}
		});

		// Add Misc (includes full Extra Gratuity)
		for (let key in rateArray?.misc || {}) {
			subtotal += rateArray.misc[key]?.amount || 0
		}
	...
	private recalculateDisplayedRates() {
		...
		if (this.rateArray) {
			const rates = this.calculateBookingTotal(this.rateArray, this.Form.service_type.value, numVehicles);
		...
		if (this.Form.service_type.value == 'round_trip' && this.returnRateArray) {
			const r_rates = this.calculateBookingTotal(this.returnRateArray, this.Form.service_type.value, numVehicles);
	...
	getValidMapCoordinatePair(latitude: any, longitude: any): { lat: number, lng: number } | null {
		const lat = this.getNumericAirportCoordinate(latitude);
		const lng = this.getNumericAirportCoordinate(longitude);

		if (lat === null || lng === null) {
			return null;
		}

		// Quote-bot and preview flows sometimes use 0,0 as a placeholder, which is not a real trip point.
		if (lat === 0 && lng === 0) {
			return null;
		}
	...
	resolveMapPointFromForm(addressPrefix: string, airportPrefix: string, useAirportPoint: boolean): { lat: number, lng: number } | null {
		const airportPoint = this.getValidMapCoordinatePair(
			this.BookingForm.get(`${airportPrefix}_latitude`)?.value,
			this.BookingForm.get(`${airportPrefix}_longitude`)?.value
		);
		const addressPoint = this.getValidMapCoordinatePair(
			this.BookingForm.get(`${addressPrefix}_latitude`)?.value,
			this.BookingForm.get(`${addressPrefix}_longitude`)?.value
		);

		if (useAirportPoint && airportPoint) {
			return airportPoint;
		}
	...
	async MapController(is_return: boolean = false) {
		try {
			...
			if (is_return) {
				...
				const returnOriginPoint = this.resolveMapPointFromForm(
					'return_pickup',
					'return_pickup_airport',
					returnTransferType.includes('airport_')
				);
				const returnDestinationPoint = this.resolveMapPointFromForm(
					'return_dropoff',
					'return_dropoff_airport',
					returnTransferType.includes('_airport')
				);

				console.log('individual/create-new-booking return map resolved points', {
					return_transfer_type: returnTransferType,
					returnOriginPoint,
					returnDestinationPoint
				});

				if (!returnOriginPoint || !returnDestinationPoint) {
					console.warn('individual/create-new-booking return map skipped: unable to resolve valid coordinates', {
						return_transfer_type: returnTransferType,
						return_pickup_latitude: this.Form.return_pickup_latitude.value,
						return_pickup_longitude: this.Form.return_pickup_longitude.value,
						return_dropoff_latitude: this.Form.return_dropoff_latitude.value,
						return_dropoff_longitude: this.Form.return_dropoff_longitude.value,
				...
				origin = new google.maps.LatLng(returnOriginPoint.lat, returnOriginPoint.lng)
				destination = new google.maps.LatLng(returnDestinationPoint.lat, returnDestinationPoint.lng)
				...
				const originPoint = this.resolveMapPointFromForm(
					'pickup',
					'pickup_airport',
					transferType.includes('airport_')
				);
				const destinationPoint = this.resolveMapPointFromForm(
					'dropoff',
					'dropoff_airport',
					transferType.includes('_airport')
				);

				console.log('individual/create-new-booking outbound map resolved points', {
				...
				if (!originPoint || !destinationPoint) {
					console.warn('individual/create-new-booking outbound map skipped: unable to resolve valid coordinates', {
						transfer_type: transferType,
				...
				destination = new google.maps.LatLng(destinationPoint.lat, destinationPoint.lng)
			}
	...
	Subscriptions() {
		// Initial trigger to initialize validators
		this.BookingForm.get('transfer_type').patchValue(this.BookingForm.get('transfer_type').value);
		this.BookingForm.get('return_transfer_type').patchValue(this.BookingForm.get('return_transfer_type').value);
		...
		this.BookingForm.get('transfer_type').valueChanges.subscribe((value: string) => {
			...
			if (!this.searchSubstring(value, '_airport')) {
		...
		this.BookingForm.get('return_transfer_type').valueChanges.subscribe((value: string) => {
			...
			// Return Dropoff Address Validation
			if (!this.searchSubstring(value, '_airport')) {
				this.BookingForm.get('return_dropoff')?.setValidators([Validators.required]);
			} else {
				this.BookingForm.get('return_dropoff')?.clearValidators();
	...
	setValueByBookNow() {
		try {
			...
			console.log('individual/create-new-booking map prefill snapshot', {
			...
			this.MapController()
			if (this.Form.service_type.value == 'round_trip') {
				setTimeout(() => this.MapController(true), 2000)
			}
